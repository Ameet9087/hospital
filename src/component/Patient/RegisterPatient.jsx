...
function RegisterPatient() {
  ...
  const handleGuarantorData = (data) => {
    setFormData((prevState) => {
      if (JSON.stringify(prevState.guarantorData) !== JSON.stringify(data)) {
        return {
          ...prevState,
          guarantorData: data,
        };
  ...
  const handleRegisterPatient = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        navigate("/patient/searchpatient");
      }
    } catch (error) {
  ...
  return (
    <div className="register-patient">
      <div className="register-patient-content">
        <PatientRegistration
          sendpatientdata={handlePatientData}
          patientData={formData.patientData}
        />
        <AddressPage
          sendaddressdata={handleAddressData}
          addressData={formData.addressData}
        />
        <GuarantorPage
          sendguarantordata={handleGuarantorData}
          guarantorData={formData.guarantorData}
        />
        <InsurancePage
          sendinsurancedata={handleInsuranceData}
          insuranceData={formData.insuranceData}
        />
        <EmergencyContactPage
          sendemergencycontactdata={handleEmergencyContactData}
          emergencyData={formData.emergencyContactData}
        />
        ...
        {isCameraOpen && (
          <div className="register-patient-camera-container">
            ...
            <button
              onClick={takePicture}
              className="register-patient-capture-btn"
            >
              Take Photo
            </button>
        ...
        {!isCameraOpen && imageSrc && (
          <div className="register-patient-image-preview">
            <img
              src={imageSrc}
              alt="Captured"
              className="register-patient-captured-image"
            />
          </div>
        )}
        <button className="register-button" onClick={handleRegisterPatient}>
          Register Patient
        </button>
      </div>
    </div>
