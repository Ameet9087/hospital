...
function BookingList() {
  ...
  const handlePatientSelection = (selectedIpNo) => {
    ...
    if (selectedPatient) {
      ...
      setFormData((prev) => ({
        ...prev,
        ipAdmissionDTO: { ...prev.ipAdmissionDTO, ipAdmmissionId: selectedIpNo },
      }));
    } else {
      ...
      try {
        ...
        if (response.status === 200 && Array.isArray(response.data)) {
          const formattedPatients = response.data.map((patient) => ({
            ...
            consultant: patient.admissionUnderDoctorDetail?.consultantDoctor?.doctorName || "N/A",
  ...
  const handleMachineChange = async (e) => {
    ...
    if (selectedMachineId) {
      try {
        const response = await axios.get(`${API_BASE_URL}/ot-machines/${selectedMachineId}`);
        if (response.status === 200) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/operation-bookings`, formattedData);
      if (response.status === 200) {
          ...
          <tbody>
            {otPatientList && otPatientList.length > 0 ? (
              otPatientList.map((booking, index) => (
                <tr key={index}>
                  <td>{index + 1}</td>
                  <td>{booking?.ipAdmissionDTO?.patient?.firstName} {booking?.ipAdmissionDTO?.patient?.lastName}</td>
                  <td>{`${booking?.ipAdmissionDTO?.patient?.age || "N/A"}/${booking?.ipAdmissionDTO?.patient?.gender || "N/A"
                    }`}</td>
                  <td>
                    {booking.otDate
          ...
          <div className="booking-list-form-panels">
            <div className="">
              ...
              <div className="booking-list-patient-search-dropdown">
                <label>IP No:</label>
                <select name="ipAdmissionDTO.ipAdmmissionId" value={formData.ipAdmissionDTO.ipAdmmissionId}

                  onChange={(e) => handlePatientSelection(e.target.value)}>
                  <option value="">Select Patient</option>
                  {inPatients.map((patient) => (
                    <option key={patient.ipAdmmissionId} value={patient.ipAdmmissionId}>
              ...
              <div className="booking-list-patient-search-dropdown">
                ...
                <select value={machineName} onChange={handleMachineChange}>
                  ...
                  {otMachines.map((machine) => (
                    <option key={machine.otMachineId} value={machine.otMachineId}>
                      {machine.machineName}
                    </option>
                  ))}
                </select>
              ...
              <div className="booking-list-patient-search-dropdown">
                ...
                <input type="date" value={signInDate} onChange={(e) => setSignInDate(e.target.value)} />
              </div>
              ...
              <div className="booking-list-patient-search-dropdown">
                ...
                <input type="date" value={shiftWardToDate} onChange={(e) => setShiftWardToDate(e.target.value)} />
              </div>

              <div className="booking-list-patient-search-dropdown">
                <label>Shifted Ward Time:</label>
                <input type="time" value={shiftToWardTime} onChange={(e) => setShiftToWardTime(e.target.value)} />
              </div>
            ...
            <div className="booking-list-form-panel">
              ...
              <div className="booking-list-patient-search-dropdown">
                ...
                <input type="date" value={signOutDate} onChange={(e) => setSignOutDate(e.target.value)} />
              </div>
              ...
              <div className="booking-list-patient-search-dropdown">
                ...
                <input type="time" value={orEntryTime} onChange={(e) => setOrEntryTime(e.target.value)} />
              </div>
              <div className="booking-list-patient-search-dropdown">
                <label>OT Date:</label>
              ...
              <div className="booking-list-patient-search-dropdown">
                ...
                <input type="text" value={otProcedure} onChange={(e) => setOtProcedure(e.target.value)} />
              </div>
              <div className="booking-list-patient-search-dropdown">
