...
const GoodReceiptComponent = () => {
  ...
  // Save a New Good Receipt
  const handleSaveReceipt = async (receiptData) => {
    try {
      await axios.post(`${API_BASE_URL}/pharmacy-good-receipt`, receiptData, {
  ...
  return (
    <div className="purchase-order-container">
      <div className="purchase-order-header">
        <button className="purchase-order-add-purchase-order-button" onClick={handleOpenModal}>
          + Add New Good Receipt
        </button>

        <div className="purchase-order-status-filters">
          <label>List by Status:</label>
          <label><input type="radio" name="status" /> Completed</label>
      ...
      <div className="purchase-data-order">
        <div className="purchase-order-date-range">
          ...
          <input type="date" id="from-date" value={dateRange.from} onChange={(e) => setDateRange((prev) => ({ ...prev, from: e.target.value }))} />
          <label htmlFor="to-date">To:</label>
          <input type="date" id="to-date" value={dateRange.to} onChange={(e) => setDateRange((prev) => ({ ...prev, to: e.target.value }))} />
        </div>
      </div>

      <div className="purchase-order-search-container">
        <input type="text" className="purchase-order-search-box" placeholder="Search..." value={searchText} onChange={(e) => setSearchText(e.target.value)} />
      ...
      <table ref={tableRef}>
        <thead>
          <tr>
            {['G.R. No', 'GR Date', 'Supplier Bill Date', 'Bill No', 'Supplier Name', 'Sub Total', 'Discount Amount', 'VAT Amount', 'Total Amount', 'Remark']
              .map((header, index) => (
        ...
        <tbody>
          {loading ? (
            <tr><td colSpan="10" className="purchase-order-no-rows">Loading...</td></tr>
          ) : filteredReceipts.length > 0 ? (
            filteredReceipts.map((receipt) => (
              <tr key={receipt.goodReceiptId}>
            ...
            <tr><td colSpan="10">No Rows To Show</td></tr>
          )}
        </tbody>
      </table>
